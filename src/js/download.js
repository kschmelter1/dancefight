...
if (!ieVersion()) {
  $(document).ready(function () {
    var dlShadow = document.getElementById('cubeShadow');
    var dlCube = document.getElementById('cube');
    if (dlCube) {
    /*** Start Draggable Cube ***/
  ...
  // Adds vendor prefixes to transforms
  var prefix = (function () {
  ...
  function Viewport(data) {
    ...
    this.distanceX = 0;
    this.distanceY = 0;
    this.positionX = 1133; // Transform rotateY
    this.positionY = 127; // Transform rotateX
    this.positionZ = 7;
    this.torqueX = 0;
    this.torqueY = 0;
    ...
    bindEvent(dlCube, 'mousedown', function() {
      self.down = true;
    });
    ...
    bindEvent(dlCube, 'touchstart', function(e) {
      self.down = true;
  ...
  } // End Function Viewport

  var canSpin = true;
  function spinTimer(spinSwitch) {
    canSpin = !spinSwitch;
    setTimeout(spinTimer, 1500, canSpin);
  }
  setTimeout(spinTimer, 1500, canSpin);
  ...
  Viewport.prototype.animate = function() {
    ...
    if(this.down) {
      ...
      if (canSpin) {
        // Automatically spins cube

        this.torqueY -= .15 * this.sensivity;
      }
      this.torqueX += .25 * this.sensivity;
    }
    ...
    this.element.style[userPrefix.js + 'Transform'] = 'rotateX(' + this.positionY + 'deg) rotateY(' + this.positionX + 'deg) rotateZ(' + this.positionZ + 'deg)';
  ...
  } //End Viewport.prototype.animate
